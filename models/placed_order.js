...
const personalDetails = (key, details) => ({
  /*
  I am using lodash to verify if the parameter exists inside the object (for example billing_address.country_code).
...
const placedOrder = (value) => {
  const payload = {
    ...
    customer_properties: {
      ...
      $address1: (typeof value.customer.default_address !== 'undefined') ? value.customer.default_address.address1 : '', 
      /* Since the following parameters differ from shipping and billing address, 
      PersonalDetails function is not used here. It may be substituted into it's own function but there's only 6 repetitions
      */
    ...
    properties: {
      ...
      Categories: [].concat([], ...value.line_items.map((item) => item.properties)), 
      //There is a field called properties returned from Shopify, which usually seems to be an empty array. I am unsure if it matches with Categories, but if not it may be removed.
      ItemNames: value.line_items.map((itemName) => itemName.name), 
      // I map through each line item and return the name attribute
      'Discount Code': (value.discount_codes.length > 0) ? (value.discount_codes.map(result => result.code)).toString() : '', 
      /* It looks like Discount Code can only take a string, however Shopify may return multiple discount codes. 
      I check for the length, if it greater than zero I loop through and return codes as comma separated string
      */
      'Discount Value': value.total_discounts,
      Items: value.line_items.map((item) => ({ 
        // I map through each lineItem and return an object with required properties
        ProductID: item.id,
        SKU: item.sku,
        ...
        //Please see ProductURL under orderedProduct
        Categories: item.properties, //Same as Categories above
      })),
      billing_address: personalDetails('billing_address', value), 
      // Both Billing and Shipping objects contain the same data, so it is useful to put it in a function
  ...
  pushDataToKlaviyo(payload)
    ...
    /* For each order, I push the payload to Klaviyo. 
       If an error occurs (such as 0 is being returned instead of 1, 
       and error is thrown and caught in the promise)
    */
};
